...
public class NotificationService {
    ...
    public boolean isRecentlySent(
        ...
        Optional<NotificationEntity> last =
                notificationRepository.findTopByUser_IdAndCompany_IdAndTypeOrderByDateDesc(
                        userId,
                        companyId,
                        type.name());
    ...
    public void sendNotification(
        ...
        if (content.isBlank()) {
            log.info(
                    "[NOTI] content blank - skip | userId={}, companyId={}, type={}",
        ...
        log.info(
                "[NOTI] saved | userId={}, companyId={}, type={}",
                user.getId(), company.getId(), type
        );
        ...
        if (fcmToken == null || fcmToken.isBlank()) {
            log.warn(
                    "[FCM] skip send - token missing | userId={}",
                    user.getId()
            );
            return;
        }
        ...
        log.info(
                "[FCM] calling sendPush | userId={}, type={}",
                user.getId(), type
        );
